...
E_NAME=asruR0
DATA_DIR=data/xtrack/asruR0_0_tagged_0n1best_nowcn_xtrack/
#"data/xtrack/${E_NAME}_0_tagged_0n1best_nowcn_xtrack/"
TMP_DIR=tmp.R2


for dataset in dev; do
    TRACK_FILE=/a/SSD/zilka/${TMP_DIR}/${E_NAME}.${dataset}.goals.x.json
    TRACK_LOG=/a/SSD/zilka/${TMP_DIR}/${E_NAME}.${dataset}.goals.x.log
    SCORE_FILE=/a/SSD/zilka/${TMP_DIR}/${E_NAME}.${dataset}.goals.x.csv
    RESULTS_FILE=/a/SSD/zilka/${TMP_DIR}/${E_NAME}.${dataset}.results.x.txt
    ...
    (
        ...
        for i in $(seq 0 9); do
            for slot in food area pricerange; do # method req_food req_area req_pricerange req_name req_phone req_addr req_postcode req_signature; do
                echo "--params_file /a/SSD/zilka/${TMP_DIR}/${E_NAME}_${i}/${slot}/params.final.p"
            done
        done
